...
export const PayrollPage: React.FC<PayrollPageProps> = ({ currentRole = 'ADMIN' }) => {
  ...
  const employeesPayroll = [
    { id: 'EMP-1001', name: 'Janani Devi', role: 'Software Engineer', dept: 'Engineering', wage: 50000, net: 42300, status: 'Processed' },
    { id: 'EMP0012', name: 'John Doe', role: 'Senior Developer', dept: 'Engineering', wage: 75000, net: 63500, status: 'Processed' },
    { id: 'EMP0013', name: 'Sarah Smith', role: 'Product Designer', dept: 'Design', wage: 60000, net: 50800, status: 'Processed' },
    { id: 'EMP0014', name: 'Michael Brown', role: 'HR Executive', dept: 'Human Resources', wage: 45000, net: 38100, status: 'Processed' },
  ];

  if (!salaryData) return null;

  const basic = salaryData.components.find((c) => c.name.includes('Basic'))?.amount || 25000;
  const hra = salaryData.components.find((c) => c.name.includes('HRA'))?.amount || 12500;
  const std = salaryData.components.find((c) => c.name.includes('Standard'))?.amount || 4000;
  const bonus = salaryData.components.find((c) => c.name.includes('Bonus'))?.amount || 2500;
  const lta = salaryData.components.find((c) => c.name.includes('LTA'))?.amount || 2000;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-blue-900 via-indigo-950 to-slate-900 p-6 sm:p-8 rounded-3xl text-white shadow-xl">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="bg-white/10 border border-white/20 text-white rounded-xl px-3.5 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer"
          >
          ...
          <button
            ...
            className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white px-4 py-2.5 rounded-xl font-semibold text-xs flex items-center gap-1.5 transition-all shadow-md cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-5 text-white shadow-lg shadow-emerald-600/20">
          <span className="text-xs font-bold text-emerald-100 uppercase tracking-wider block mb-1">Net Take-Home Pay</span>
          <span className="text-2xl font-extrabold">₹{salaryData.takeHomeSalary.toLocaleString('en-IN')}</span>
          <span className="text-[11px] text-emerald-100 block mt-1">Disbursed on 30th of each month</span>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {employeesPayroll.map((emp) => (
                  <tr key={emp.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right font-extrabold text-emerald-600">₹{emp.net.toLocaleString('en-IN')}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-4">
          <h3 className="font-bold text-slate-800 text-base border-b border-slate-100 pb-3 flex items-center justify-between">
            ...
            <span className="text-xs text-blue-600 font-bold">₹{salaryData.grossSalary.toLocaleString('en-IN')} / mo</span>
          </h3>

          <div className="space-y-2.5 text-xs">
            <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-semibold text-slate-700">Basic Salary (50%)</span>
              <span className="font-mono font-bold text-slate-900">₹{basic.toLocaleString('en-IN')}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-semibold text-slate-700">House Rent Allowance - HRA (50% of Basic)</span>
              <span className="font-mono font-bold text-slate-900">₹{hra.toLocaleString('en-IN')}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-semibold text-slate-700">Standard Allowance (16%)</span>
              <span className="font-mono font-bold text-slate-900">₹{std.toLocaleString('en-IN')}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-semibold text-slate-700">Performance Bonus (10%)</span>
              <span className="font-mono font-bold text-slate-900">₹{bonus.toLocaleString('en-IN')}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50">
              <span className="font-semibold text-slate-700">Leave Travel Allowance - LTA (8%)</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-2.5 text-xs">
            ...
            <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 text-[11px] text-blue-700 mt-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden p-8 space-y-6 animate-in zoom-in-95">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 pb-6">
              <div>
                <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Dayflow Solutions Pvt. Ltd.</h2>
                <p className="text-xs text-slate-500">Official Salary Slip for the month of {selectedMonth}</p>
              </div>
              <div className="text-right">
                <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-800 text-xs font-bold">CONFIDENTIAL</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div>
                ...
                <span className="font-mono font-bold text-slate-700">EMP-1001</span>
              </div>
              <div>
                <span className="text-slate-400 block">Bank Account:</span>
                <span className="font-mono font-bold text-slate-700">HDFC Bank ••••• 4321</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 text-xs">
              <div className="space-y-2">
                ...
                <div className="flex justify-between"><span>Bonus</span><span>₹{bonus.toLocaleString('en-IN')}</span></div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between"><span>TDS</span><span>₹{salaryData.deductions.incomeTaxTds.toLocaleString('en-IN')}</span></div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl text-xs font-semibold bg-blue-600 text-white hover:bg-blue-700 flex items-center gap-1.5 shadow-md cursor-pointer"
              >
